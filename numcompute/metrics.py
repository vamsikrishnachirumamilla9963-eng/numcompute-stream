...
def _prf(
    ...
    y_t = y_true[:, np.newaxis]
    y_p = y_pred[:, np.newaxis]
    c = classes[np.newaxis, :]
    ...
    if average == "micro":
        ...
        pm = float(tp_s / (tp_s + fp_s)) if (tp_s + fp_s) > 0 else 0.0
        rm = float(tp_s / (tp_s + fn_s)) if (tp_s + fn_s) > 0 else 0.0
        ...
        d = pm + rm
        return float(2 * pm * rm / d) if d > 0 else 0.0
    ...
    denom = p + r
    return float(np.mean(np.where(denom > 0, 2 * p * r / denom, 0.0)))
...
def roc_curve(
    ...
    thresh = np.concatenate([[thresh[0] + 1.0], thresh])
...
class StreamingClassificationMetrics:
    ...
    recall, f1, confusion matrix, and binary AUC can be computed over chunks.
    """
